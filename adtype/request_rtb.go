...
var (
	...
	emptyCompany   = admodels.Company{}
	emptyZone      = admodels.Zone{Comp: &emptyCompany}
)
...
func (r *RTBRequest) LoadBytes(data []byte) error {
	if err := json.Unmarshal(data, &r.RTBRequest); err != nil {
		return err
	}
	r.UpdateState()
	return nil
...
func (r *RTBRequest) UserID() string {
	uuid := r.RTBRequest.User.BuyerUID
	if len(uuid) < 1 {
...
func (r *RTBRequest) imp(target admodels.Target, imp *openrtb.Impression) *Impression {
	var (
