...
func (g generator) Event(event events.Type, status uint8, response adtype.Responser, it adtype.ResponserItem) (*events.Event, error) {
	...
	return &events.Event{
		...
		PurchaseViewPrice:  imp.PurchasePrice(admodels.ActionImpression), // Price of of the view of source traffic cost
		PurchaseClickPrice: imp.PurchasePrice(admodels.ActionClick),      // Price of of the click of source traffic cost
		PurchaseLeadPrice:  imp.PurchasePrice(admodels.ActionLead),       // Price of of the lead of source traffic cost
